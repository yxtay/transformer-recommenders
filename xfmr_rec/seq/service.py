...
class Model:
    ...
    def embed(self, queries: list[Query]) -> list[Query]:
        ...
        embeddings = self.model(query_texts)["sentence_embedding"].numpy(force=True)
...
class Service:
    ...
    @bentoml.api(batchable=True)
    @logger.catch(reraise=True)
    async def embed_queries(self, queries: list[Query]) -> list[Query]:
        return await self.model.to_async.embed(queries)
